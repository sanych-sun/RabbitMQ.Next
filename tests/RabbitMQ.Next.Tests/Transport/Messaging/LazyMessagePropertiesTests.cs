...
namespace RabbitMQ.Next.Tests.Transport.Messaging
{
    public class LazyMessagePropertiesTests
    {
        ...
        {
            var props = new LazyMessageProperties(new ReadOnlySequence<byte>(source));
        ...
        {
            var props = new LazyMessageProperties(new ReadOnlySequence<byte>(new byte[] { 0b_00000000, 0b_00000000 }));
